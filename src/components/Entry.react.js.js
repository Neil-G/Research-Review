    ...
    console.log("entry", entry)
      ...
      <div style={{  paddingBottom: '0px', padding: '0 42px 6px 12px' }}>
        {/* ENTRY TITLE */}
          ...
          {/* ENTRY DESCRIPTION */}
          <h6 style={{ marginBottom: '5px', lineHeight: '1' }}> {entry.description } </h6>
             ...
             * {entry.type} * <span style={{ cursor: 'pointer' }} onClick={ this.setState.bind(this, { expanded: !this.state.expanded }) }> ({entry.terms && entry.terms.length || 0}/{entry.points && entry.points.length || 0}) </span>
             {/* EXPANDING SECTION */}
              <span 
                ...
                style={{ maxHeight: this.state.expanded ? 1000 : 0, overflow: 'hidden', transition: 'all 0.8s', padding: ' 0 12px', color: 'tomato', paddingBottom: '6px', paddingLeft: '0px' }}>
                { '* ' }   
                { 
